...
import PtAssistantPage from '../../../src/pages/pt/PtAssistantPage';
...
it('hiển thị safety/citation và chỉ apply sau approve', async () => {
  vi.mocked(api.get).mockImplementation(async (path) =>
    path.startsWith('/api/assistant/suggestions')
      ? {
          data: [
            {
              _id: 's1',
  ...
  vi.mocked(api.patch).mockResolvedValue({ data: { reviewStatus: 'APPROVED' }, message: 'Đã duyệt.' });
