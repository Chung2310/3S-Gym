...
export default function AiWorkoutWizard({ open, customers, onClose, onGenerated }: Props) {
  ...
  if (!open) return null;
  const analyze = async () => { if (!customerId) return setError('Vui lòng chọn học viên.'); setLoading(true); setError(''); try { const result = await api.post<Proposal>('/api/ai/workout-proposals', { customerId }); setProposal(result.data); setStep(1); } catch (cause) { setError(errorMessage(cause)); } finally { setLoading(false); } };
  ...
  const continueToGeneration = () => {
    if (!proposal) return;
    if (!Number.isInteger(proposal.durationWeeks) || proposal.durationWeeks < 4 || proposal.durationWeeks > 12) return setError('Số tuần phải từ 4 đến 12.');
    if (!Number.isInteger(proposal.sessionsPerWeek) || proposal.sessionsPerWeek < 1 || proposal.sessionsPerWeek > 7) return setError('Số buổi mỗi tuần phải từ 1 đến 7.');
    if (!Number.isInteger(proposal.minutesPerSession) || proposal.minutesPerSession < 15 || proposal.minutesPerSession > 240) return setError('Số phút mỗi buổi phải từ 15 đến 240.');
    setError('');
    setStep(3);
  };
  const primaryAction = () => {
