...
interface WorkoutBuilderProps { onSaved: () => void; template?: WorkoutTemplate | null }

export default function WorkoutBuilder({ onSaved, template = null }: WorkoutBuilderProps) {
  ...
  return <form className="panel" onSubmit={submit}><h2>Tạo giáo án mẫu</h2><div className="form-grid"><label className="field"><span>Tên giáo án</span><input aria-label="Tên giáo án" value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label className="field"><span>Mục tiêu</span><input aria-label="Mục tiêu" value={goal} onChange={(event) => setGoal(event.target.value)} required /></label><label className="field"><span>Cấp độ</span><select aria-label="Cấp độ giáo án" value={level} onChange={(event) => setLevel(event.target.value)}><option value="BEGINNER">Cơ bản</option><option value="INTERMEDIATE">Trung cấp</option><option value="ADVANCED">Nâng cao</option></select></label></div>
    {sessions.map((session, sessionIndex) => <fieldset key={sessionIndex}><legend>Buổi {sessionIndex + 1}</legend><label className="field"><span>Tên buổi</span><input aria-label={`Tên buổi ${sessionIndex + 1}`} value={session.name} onChange={(event) => updateSessionName(sessionIndex, event.target.value)} required /></label><div className="inline-actions">{library.map((exercise) => <button className="button button-secondary" type="button" key={exercise._id} onClick={() => addExercise(sessionIndex, exercise)}>Thêm bài {exercise.name}</button>)}</div>{session.exercises.map((exercise, index) => <p key={`${exercise.exerciseId}-${index}`}>{exercise.name} · {exercise.sets} × {exercise.reps}</p>)}</fieldset>)}
    <div className="modal-actions"><button className="button button-secondary" type="button" onClick={() => setSessions((current) => [...current, { name: '', exercises: [] }])}>Thêm buổi tập</button><button className="button button-primary" type="submit" disabled={loading}>{loading ? 'Đang lưu...' : 'Lưu giáo án'}</button></div>
  </form>;
}
