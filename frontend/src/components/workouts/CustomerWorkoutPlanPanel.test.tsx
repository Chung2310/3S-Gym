...
it('lọc và công bố giáo án khách hàng sau bước xác nhận', async () => {
  vi.mocked(api.get).mockResolvedValue({ data: [{ _id: 'p1', customerId: 'c1', title: 'Plan A', startDate: '', endDate: '', sessions: [], status: 'DRAFT' }], meta: { page: 1, totalPages: 1 }, message: '' });
  vi.mocked(api.patch).mockResolvedValue({ data: {}, message: 'Đã công bố.' });
  const user = userEvent.setup();
  render(<ToastProvider><CustomerWorkoutPlanPanel /></ToastProvider>);
  expect((await screen.findAllByText('Plan A'))[0]).toBeVisible();
  await user.type(screen.getByLabelText('Lọc theo mã khách hàng'), 'c1');
