...
export default function RoadmapForm({ onSaved, onCancel }: RoadmapFormProps) {
  ...
  return <form className="panel" onSubmit={submit}>
    <div className="form-grid"><label className="field"><span>Mã khách hàng</span><input aria-label="Mã khách hàng" placeholder="Nhập mã khách hàng (Customer ID)..." value={customerId} onChange={(event) => setCustomerId(event.target.value)} required /></label><label className="field"><span>Tên roadmap</span><input aria-label="Tên roadmap" placeholder="Ví dụ: Lộ trình Giảm mỡ 12 tuần..." value={title} onChange={(event) => setTitle(event.target.value)} required /></label></div>
    {phases.map((phase, phaseIndex) => <fieldset className="panel" key={phaseIndex}><legend>Phase {phaseIndex + 1}</legend><div className="form-grid"><label className="field"><span>Thứ tự</span><input aria-label={`Thứ tự phase ${phaseIndex + 1}`} type="number" min="1" placeholder="1" value={phase.order} onChange={(event) => updatePhase(phaseIndex, { order: Number(event.target.value) })} required /></label><label className="field"><span>Tên phase</span><input aria-label={`Tên phase ${phaseIndex + 1}`} placeholder="Ví dụ: Giai đoạn 1 - Thích nghi & Nền tảng..." value={phase.name} onChange={(event) => updatePhase(phaseIndex, { name: event.target.value })} required /></label><label className="field"><span>Thời lượng (tuần)</span><input aria-label={`Thời lượng phase ${phaseIndex + 1}`} type="number" min="1" placeholder="4" value={phase.durationWeeks} onChange={(event) => updatePhase(phaseIndex, { durationWeeks: Number(event.target.value) })} required /></label></div>
      {phase.weeks.map((week, weekIndex) => <label className="field" key={weekIndex}><span>Trọng tâm tuần {week.week}</span><input aria-label={`Trọng tâm tuần ${week.week} phase ${phaseIndex + 1}`} placeholder="Ví dụ: Kỹ thuật Squat & Hít thở..." value={week.focus} onChange={(event) => updateWeek(phaseIndex, weekIndex, event.target.value)} required /></label>)}
