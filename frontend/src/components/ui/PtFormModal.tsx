...
export default function PtFormModal({ open, pt, onClose, onSaved }: PtFormModalProps) {
  ...
  return <ProfileFormModal open={open} title={editing ? 'Sửa PT' : 'Thêm PT'} description="Nhập thông tin hồ sơ và tài khoản huấn luyện viên." dirty={dirty} loading={loading} submitLabel={editing ? 'Lưu thay đổi' : 'Tạo PT'} onClose={onClose} onSubmit={submit}>
            ...
            <section className="profile-form-section"><h3>Liên hệ</h3><div className="profile-form-grid"><FormField label="Số điện thoại" name="phone" value={form.phone} onChange={change} required /><FormField label="Email" name="email" type="email" value={form.email} onChange={change} /><div className="grid-full-width"><FormField label="Địa chỉ" name="address" value={form.address} onChange={change} /></div></div></section>
            <section className="profile-form-section"><h3>Chuyên môn</h3><div className="profile-form-grid"><FormField label="Chuyên môn" name="specialization" value={form.specialization} onChange={change} /><FormField label="Số năm kinh nghiệm" name="yearsOfExperience" type="number" min={0} max={80} step={1} value={form.yearsOfExperience} onChange={change} /><div className="grid-full-width"><FormField label="Chứng chỉ" name="certificates" as="textarea" rows={3} placeholder="Mỗi chứng chỉ một dòng" value={form.certificates} onChange={change} /></div><div className="grid-full-width"><FormField label="Giới thiệu" name="bio" as="textarea" rows={3} maxLength={1000} value={form.bio} onChange={change} /></div></div></section>
