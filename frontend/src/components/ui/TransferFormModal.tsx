...
export default function TransferFormModal({ open, transfer, onClose, onSaved }: TransferFormModalProps) {
  ...
  return <FormModal open={open} title={editing ? 'Sửa yêu cầu chuyển PT' : 'Tạo yêu cầu chuyển PT'} dirty={dirty} onClose={onClose} onSubmit={submit} submitLabel={editing ? 'Lưu thay đổi' : 'Gửi yêu cầu'}>
    <section className="profile-form-section"><div className="profile-form-grid"><FormField label="Mã khách hàng" name="transferCustomerId" placeholder="Nhập mã khách hàng (Customer ID)..." value={form.customerId} onChange={(e) => setForm({ ...form, customerId: e.target.value })} readOnly={editing} required /><FormField label="Mã PT nhận" name="transferToPtId" placeholder="Nhập mã PT tiếp nhận..." value={form.toPtId} onChange={(e) => setForm({ ...form, toPtId: e.target.value })} required /><FormField label="Lý do chuyển" name="transferReason" placeholder="Nhập lý do chuyển giao khách hàng..." value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} required /></div></section>
  </FormModal>;
}
