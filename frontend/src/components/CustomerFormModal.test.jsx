...
describe('CustomerFormModal', () => {
  ...
  it('tạo khách với payload đã chuẩn hóa', async () => {
    ...
    api.post.mockResolvedValue({ message: 'Tạo khách hàng thành công.', data: { _id: 'customer-1' } });
  ...
  it('sửa khách bằng PATCH với dữ liệu điền sẵn', async () => {
    const user = userEvent.setup();
    api.patch.mockResolvedValue({ message: 'Cập nhật khách hàng thành công.', data: { _id: 'customer-1', fullName: 'Khách B' } });
