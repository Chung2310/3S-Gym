...
export default function PtPackageManagerModal({ open, customer, onClose }) {
  ...
  const load = async (page = 1) => {
    ...
    try {
      const result = await api.get(`/api/customers/${customer._id}/packages?page=${page}&limit=20${statusFilter ? `&status=${statusFilter}` : ''}`);
  ...
  return <>
    <FormModal open={open} title={`Gói PT của ${customer?.fullName || 'khách hàng'}`} description="Tạo và quản lý số buổi tập của khách hàng." dirty={Boolean(form.name || form.totalSessions || form.startDate || form.endDate)} loading={loading} submitLabel={editing ? 'Lưu thay đổi' : 'Thêm gói PT'} onClose={onClose} onSubmit={submit}>
      ...
      <div className="package-list"><h3>Các gói đã tạo</h3>
        <div className="filter-bar"><label className="field"><span>Lọc trạng thái gói</span><select aria-label="Lọc trạng thái gói PT" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">Tất cả trạng thái</option><option value="ACTIVE">Đang hoạt động</option><option value="EXPIRED">Hết hạn</option><option value="COMPLETED">Hoàn thành</option><option value="CANCELLED">Đã hủy</option></select></label><button type="button" className="button button-secondary" onClick={() => load(1)}>Lọc gói</button></div>
