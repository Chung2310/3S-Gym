...
export default function CustomerWorkoutPlanTab({ customerId, customerName }: Props) {
  ...
  return <div className="space-y-4">
    {state.active ? <section className="rounded-2xl border border-sky-200 bg-gradient-to-br from-white to-sky-50 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="mb-1 text-xs font-bold uppercase tracking-wider text-secondary">Giáo án đang áp dụng</p><h3 className="font-oswald text-2xl font-bold uppercase text-primary">{state.active.title}</h3><p className="mt-1 text-sm text-slate-600">{state.active.goal} · {state.active.level} · {state.active.durationDays} ngày</p><p className="mt-2 flex items-center gap-1 text-xs text-slate-500"><CalendarDays size={14} /> Gán cho {customerName}</p></div><div className="flex gap-2"><button type="button" className="button button-secondary" onClick={() => setPickerOpen(true)}>Thay giáo án</button><button type="button" className="button button-primary" onClick={() => navigate(`/pt/customers/${customerId}/workout-plans/${state.active?._id}/edit`)}>Mở Studio</button></div></div></section> : <section className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center"><Dumbbell className="mx-auto mb-3 text-slate-400" size={38} /><h3 className="font-semibold text-slate-900">Khách hàng chưa có giáo án</h3><p className="mb-4 mt-1 text-sm text-slate-500">Gán một giáo án mẫu để bắt đầu cá nhân hóa.</p><button type="button" className="button button-primary" onClick={() => setPickerOpen(true)}>Gán giáo án</button></section>}
