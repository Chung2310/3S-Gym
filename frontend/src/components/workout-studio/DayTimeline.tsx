...
export default function DayTimeline(props: Props) {
  ...
  return (
    <main ref={props.wrapperRef} className="studio-timeline-panel">
      <header className="studio-timeline-header">
        <h2>Ngày {props.activeDay} · 00:00–24:00</h2>
        {props.preview && (
          <p aria-live="polite" className={`studio-move-preview ${props.preview.valid ? 'is-valid' : 'is-invalid'}`}>
      ...
      {props.items.length > 0 && (
        <div className="studio-touch-schedule-list" role="group" aria-label="Bài tập đã xếp lịch">
          {props.items.map((item) => (
            <button
              type="button"
              className="studio-touch-schedule-action"
              key={item.id}
              onClick={() => props.onSelect(item.id)}
              aria-label={`Mở thuộc tính: ${item.name}, ${formatMinute(item.startMinute)} đến ${formatMinute(item.startMinute + item.durationMinutes)}`}
            >
              <strong>{item.name}</strong>
              <span>{formatMinute(item.startMinute)}–{formatMinute(item.startMinute + item.durationMinutes)}</span>
            </button>
          ))}
        </div>
      )}
