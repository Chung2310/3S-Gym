...
function ScheduledExerciseCard({ item, selected, preview, onSelect, onMoveStart, onResizeStart, onKeyboardMove }: { item: ScheduledExercise; selected: boolean; preview?: MovePreview; onSelect: (id: string) => void; onMoveStart: Props['onMoveStart']; onResizeStart: Props['onResizeStart']; onKeyboardMove: Props['onKeyboardMove'] }) {
  ...
  return <button draggable onDragStart={(event) => event.dataTransfer.setData('scheduleId', item.id)} className={`studio-scheduled-item ${selected ? 'is-selected' : ''} ${preview ? `is-preview ${preview.valid ? 'is-valid' : 'is-invalid'}` : ''}`} style={positionStyle(displayStart, item.durationMinutes)} onPointerDown={(event) => onMoveStart(event, item)} onClick={() => onSelect(item.id)} onKeyDown={(event) => { if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown') return; event.preventDefault(); onKeyboardMove(item, (event.shiftKey ? 60 : 15) * (event.key === 'ArrowDown' ? 1 : -1)); }}><span className="studio-scheduled-content"><strong>{item.name}</strong><span>{formatMinute(displayStart)}–{formatMinute(displayStart + item.durationMinutes)} · {item.durationMinutes} phút</span></span><i className="studio-resize-handle" aria-label="Thay đổi thời lượng" onPointerDown={(event) => onResizeStart(event, item)} /></button>;
}
