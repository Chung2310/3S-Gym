...
function AdminView() {
  ...
  const [formPt, setFormPt] = useState(null);
  const [deletePt, setDeletePt] = useState(null);
  const load = useCallback(async (page = 1) => {
    try {
      const result = await api.get(`/api/users?page=${page}&limit=20&role=PT&keyword=${encodeURIComponent(keyword)}${status ? `&status=${status}` : ''}`);
  ...
  return <>
    ...
    <ConfirmModal open={Boolean(deletePt)} title="Xóa PT vĩnh viễn?" description="PT chỉ được xóa sau khi đã chuyển hết khách sang PT khác." danger confirmLabel="Xóa vĩnh viễn" onClose={() => setDeletePt(null)} onConfirm={async () => { try { const result = await api.delete(`/api/users/${deletePt._id}`); toast.success(result.message); setDeletePt(null); load(); } catch (error) { toast.error(error.message); } }} />
  </>;
}
...
function PtView() {
  ...
  const load = useCallback(async (page = 1) => { try { const params = new URLSearchParams({ page: String(page), limit: '20' }); if (tab === 'customers' && keyword) params.set('keyword', keyword); if (tab === 'customers' && customerStatus) params.set('status', customerStatus); if (tab !== 'customers' && statusFilter) params.set('status', statusFilter); if (tab !== 'customers' && customerFilter) params.set('customerId', customerFilter); const result = await api.get(`/api/${tab}?${params}`); setItems(result.data); setMeta(result.meta); } catch (error) { toast.error(error.message); } }, [tab, keyword, customerStatus, statusFilter, customerFilter, toast]);
  useEffect(() => { load(); }, [load]);
  const publish = async () => { try { const result = await api.patch(`/api/${tab}/${confirm._id}/${confirm.status === 'PUBLISHED' ? 'unpublish' : 'publish'}`); toast.success(result.message); setConfirm(null); load(); } catch (error) { toast.error(error.message); } };
  ...
  const columns = useMemo(() => {
    ...
    return [{ key: 'title', label: tab === 'inbody' ? 'Ngày đo' : 'Tên nội dung', render: (item) => item.title || new Date(item.measurementDate).toLocaleDateString('vi-VN') }, { key: 'customerId', label: 'Khách hàng' }, { key: 'status', label: 'Trạng thái', render: (item) => <StatusBadge status={item.status} /> }];
  }, [tab]);
  ...
  return <>
    ...
    <ContentFormModal open={showForm && !['customers', 'transfers'].includes(tab)} resource={tab} item={formItem} onClose={() => setShowForm(false)} onSaved={() => { setShowForm(false); load(); }} />
    ...
    <ConfirmModal open={Boolean(deleteTarget)} title="Xóa vĩnh viễn?" description="Dữ liệu đã xóa không thể khôi phục." danger confirmLabel="Xóa vĩnh viễn" onClose={() => setDeleteTarget(null)} onConfirm={async () => { try { const path = tab === 'customers' ? `/api/customers/${deleteTarget._id}` : `/api/${tab}/${deleteTarget._id}`; const result = await api.delete(path); toast.success(result.message); setDeleteTarget(null); load(); } catch (error) { toast.error(error.message); } }} />
  </>;
}

function CustomerView() {
  const toast = useToast();
  const [content, setContent] = useState({ inbody: [], goals: [], workoutPlans: [], nutritionPlans: [] });
  useEffect(() => { api.get('/api/me/content').then((result) => setContent(result.data)).catch((error) => toast.error(error.message)); }, [toast]);
  const groups = [['inbody', 'Kết quả InBody'], ['goals', 'Mục tiêu'], ['workoutPlans', 'Giáo án'], ['nutritionPlans', 'Dinh dưỡng']];
...
export default function PortalPage({ session: providedSession }) {
  const session = providedSession || getSession();
  const user = session?.user || { username: '', role: '' };
  return <AppShell user={user}>{user.role === 'ADMIN' ? <AdminView /> : user.role === 'PT' ? <PtView /> : <CustomerView />}</AppShell>;
}
