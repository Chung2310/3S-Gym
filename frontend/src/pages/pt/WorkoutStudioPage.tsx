...
const createScheduled = (exercise: Exercise, dayNumber: number, startMinute: number): ScheduledExercise => ({ id: crypto.randomUUID(), dayNumber, startMinute, durationMinutes: 60, exerciseId: exercise._id, name: exercise.name });
...
export default function WorkoutStudioPage() {
  const { templateId, customerId, planId } = useParams(); const navigate = useNavigate(); const toast = useToast(); const [searchParams] = useSearchParams();
  ...
  const [durationDays, setDurationDays] = useState(7); const [activeDay, setActiveDay] = useState(1); const [library, setLibrary] = useState<Exercise[]>([]); const [exerciseQuery, setExerciseQuery] = useState(''); const [muscleGroup, setMuscleGroup] = useState(''); const [exerciseLevel, setExerciseLevel] = useState('');
  ...
  const selected = items.find((item) => item.id === selectedId); const dayItems = items.filter((item) => item.dayNumber === activeDay); const totalMinutes = dayItems.reduce((sum, item) => sum + item.durationMinutes, 0);
  ...
  const place = (exercise: Exercise, startMinute: number) => { const candidate = createScheduled(exercise, activeDay, Math.min(startMinute, 1380)); if (hasOverlap(items, candidate)) return toast.error('Khung giờ này đã có bài tập.'); mutate([...items, candidate]); setSelectedId(candidate.id); setInspectorOpen(true); };
  ...
  const save = async () => { if (readOnly) return; if (!title.trim() || !goal.trim() || !items.length) return toast.error('Vui lòng nhập thông tin và xếp ít nhất một bài tập.'); setSaving(true); try { const scheduledExercises = items.map(({ id: _id, ...item }) => item); const unscheduledExercises = unscheduled.map(({ id: _id, dayNumber: _dayNumber, startMinute: _startMinute, ...item }) => item); const payload = { title, goal, level, durationDays, ...metadata, scheduledExercises, unscheduledExercises }; const result = customerMode ? await api.patch<{ _id: string }>(`/api/customers/${customerId}/workout-plans/${planId}`, payload) : templateId ? await api.patch<{ _id: string }>(`/api/workout-templates/${templateId}`, payload) : await api.post<{ _id: string }>('/api/workout-templates', payload); toast.success(result.message); setDirty(false); if (!templateId && !customerMode) navigate(`/pt/my-workout-plans/${result.data._id}/edit`, { replace: true }); } catch (error) { toast.error(errorMessage(error)); } finally { setSaving(false); } };
  ...
  return <section className={`workout-studio !gap-2 ${inspectorOpen ? 'inspector-open' : ''}`}>
