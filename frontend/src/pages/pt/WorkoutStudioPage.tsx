...
export default function WorkoutStudioPage() {
  ...
  useEffect(() => {
    ...
    setItems((draft.scheduledExercises || []).map((item: ScheduledExercise) => normalizePlanExercise({ ...item, weekNumber: item.weekNumber || 1, id: crypto.randomUUID() })));
  ...
  useEffect(() => {
    ...
    api.get<StudioTemplate & { customerName?: string; customerId?: { fullName?: string } }>(resourcePath).then(({ data }) => {
      ...
      if (data.scheduledExercises?.length) setItems(data.scheduledExercises.map((item) => normalizePlanExercise({ ...item, id: crypto.randomUUID() })));
