...
export default function WorkoutStudioPage() {
  ...
  const place = (exercise: Exercise, startMinute: number) => { const candidate = createScheduled(exercise, activeWeek, activeDay, Math.min(startMinute, 1380)); if (hasOverlap(items, candidate)) return toast.error('Khung giờ này đã có bài tập.'); mutate([...items, candidate]); setSelectedId(candidate.id); setInspectorOpen(true); setStudioView('schedule'); };
  const placeUnscheduled = (item: ScheduledExercise) => { const candidate = { ...item, dayNumber: activeDay, startMinute: Math.min(480, DAY_MINUTES - item.durationMinutes) }; if (hasOverlap(items, candidate)) return toast.error('Khung giờ này đã có bài tập.'); mutate([...items, candidate]); setUnscheduled((current) => current.filter((value) => value.id !== item.id)); setSelectedId(item.id); setInspectorOpen(true); setStudioView('schedule'); };
  ...
  return <section aria-label="Workout Studio" className={`module-page workout-studio ${inspectorOpen ? 'inspector-open' : ''}`}>
    ...
    <div className="studio-workspace">
      <section className={`studio-library-region ${studioView === 'library' ? 'is-mobile-active' : ''}`} aria-label="Thư viện bài tập Studio">
      ...
      <section className={`studio-schedule-region ${studioView === 'schedule' ? 'is-mobile-active' : ''}`} aria-label="Lịch tập">
