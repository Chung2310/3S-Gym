  ...
  return (
    <section>
      <div className="section-header">
        <div>
          <h1>Roadmap</h1>
          <p>Lộ trình theo phase; phiên bản cũ chỉ dùng để tham chiếu.</p>
        </div>
        ...
        {items.length ? (
          items.map((item) => (
            <article className="panel" key={item._id}>
              ...
              <button className="button button-secondary" onClick={() => setPublishing(item)}>
                {item.status === 'PUBLISHED' ? 'Gỡ công bố' : 'Công bố roadmap'}
              </button>
            </article>
          ))
        ) : (
          <div className="empty-state">Chưa có roadmap.</div>
        )}
      </div>
      ...
      <ConfirmModal
        ...
        description="Chỉ nội dung đã công bố mới được phép hiển thị cho khách hàng khi backend hỗ trợ contract tương ứng."
