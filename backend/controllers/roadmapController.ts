...
const get = asyncHandler(async (req, res) => success(res, { message: 'Láº¥y roadmap thÃ nh cÃ´ng.', data: await service.get(req.user!, String(req.params.id)) }));
const remove = asyncHandler(async (req, res) => { await service.remove(req.user!, String(req.params.id)); return success(res, { message: 'XÃ³a roadmap thÃ nh cÃ´ng.', data: null }); });
