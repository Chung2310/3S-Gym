...
const publishKnowledge = asyncHandler(async (req, res) => success(res, { message: 'Xuất bản tài liệu thành công.', data: await knowledge.publishDocument(req.user!, String(req.params.id), req.requestId!) }));
...
const indexKnowledge = asyncHandler(async (req, res) => success(res, { message: 'Index tài liệu thành công.', data: await knowledge.indexDocument(req.user!, String(req.params.id), req.requestId!) }));
const searchKnowledge = asyncHandler(async (req, res) => success(res, { message: 'Tìm kiếm tri thức thành công.', data: await knowledge.searchPublished(req.user!, String(req.query.q || ''), 5, req.requestId!) }));
const seedStandardKnowledge = asyncHandler(async (req, res) => success(res, { message: 'Đã nạp trọn bộ tri thức chuẩn 3S-Gym vào Kho tri thức thành công.', data: await knowledge.seedStandardKnowledgeLibrary(req.user!, req.requestId!) }));

const createSuggestion = asyncHandler(async (req, res) => success(res, { status: 201, message: 'Tạo đề xuất thành công. PT cần kiểm tra trước khi sử dụng.', data: await assistant.createSuggestion(req.user!, req.body, req.requestId!) }));
...
const addConversationMessage = asyncHandler(async (req, res) => success(res, { message: 'Gửi tin nhắn thành công.', data: await assistant.addConversationMessage(req.user!, String(req.params.id), req.body, req.requestId!) }));
