...
describe('production environment', () => {
  ...
  it('.env.example exposes only the approved deployment contract', () => {
    ...
    expect(assignments.map((line) => line.split('=', 1)[0])).toEqual([
      ...
      'OPENROUTER_API_KEY', 'APP_URL',
      'VNPAY_TMN_CODE', 'VNPAY_HASH_SECRET', 'VNPAY_PAYMENT_URL', 'VNPAY_RETURN_URL', 'VNPAY_IPN_URL',
      'MOMO_PARTNER_CODE', 'MOMO_ACCESS_KEY', 'MOMO_SECRET_KEY', 'MOMO_API_URL', 'MOMO_REDIRECT_URL', 'MOMO_IPN_URL',
      'CLOUDINARY_CLOUD_NAME', 'CLOUDINARY_API_KEY', 'CLOUDINARY_API_SECRET',
    ]);
    for (const key of ['MONGODB_PASSWORD', 'JWT_SECRET', 'ADMIN_PASSWORD', 'OPENROUTER_API_KEY', 'VNPAY_HASH_SECRET', 'MOMO_ACCESS_KEY', 'MOMO_SECRET_KEY', 'CLOUDINARY_API_KEY', 'CLOUDINARY_API_SECRET']) {
