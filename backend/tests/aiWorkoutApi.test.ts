...
vi.mock('../services/aiProvider.js', () => ({ generateText: vi.fn().mockImplementation((_context: unknown, prompt: string) => Promise.resolve(prompt.includes('scheduledExercises') ? JSON.stringify({ title: 'Draft AI', goal: 'FAT_LOSS', level: 'BEGINNER', durationWeeks: 8, sessionsPerWeek: 4, minutesPerSession: 60, scheduledExercises: [{ weekNumber: 1, dayNumber: 1, startMinute: 480, durationMinutes: 60, generatedExerciseName: 'Squat không dụng cụ' }], generatedExercises: [{ name: 'Squat không dụng cụ', muscleGroup: 'LEGS', level: 'BEGINNER', defaultTrackingType: 'BODYWEIGHT', equipment: [], description: '', technique: '', commonMistakes: [], contraindications: [], variants: [] }] }) : JSON.stringify({ durationWeeks: 8, sessionsPerWeek: 4, minutesPerSession: 60, level: 'BEGINNER', trainingMethod: 'Full body', trainingSplit: 'Full body', priorityMuscleGroups: ['LEGS'], restrictions: [] }))) }));
