...
function translateDishToEnglish(name) {
...
function buildAiImagePrompt(itemNames) {
...
function getAccurateMealPlatterImage(itemNames) {
...
function parseAiMealPlanToPoster(text, targetCalories, proteinGrams) {
  if (!text) return [];
  ...
  const parseLinesToDishes = (lines) => {
    let dishes = [];
    lines.forEach((line) => {
...
async function fetchDishImageAsBase64(prompt) {
  try {
...
async function enrichPosterWithImages(posterList) {
...
router.post('/calculate', async (req, res) => {
    ...
    const activityObj = activityMap[activityLevel] || activityMap.moderate;
    ...
    let posterList = [];
    let aiTextAdvice = await fetchOpenRouterAIAdvice({
...
router.get('/meal-image', async (req, res) => {
  ...
  try {
    ...
    console.error('Pollinations AI fetch error/timeout:', err.message);
  }
