...
function validateCustomerFields(body, { partial = false, allowAssignedPt = false } = {}) {
  const errors = [];
  ...
  if (has('email') && body.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email)) errors.push({ field: 'email', message: 'Email không đúng định dạng.' });
  if (has('dateOfBirth') && body.dateOfBirth && (Number.isNaN(Date.parse(body.dateOfBirth)) || new Date(body.dateOfBirth) > new Date())) errors.push({ field: 'dateOfBirth', message: 'Ngày sinh không hợp lệ hoặc ở tương lai.' });
  if (has('gender') && !['MALE', 'FEMALE', 'OTHER'].includes(body.gender)) errors.push({ field: 'gender', message: 'Giới tính không hợp lệ.' });
  ...
  if (has('status') && !['ACTIVE', 'INACTIVE', 'LEAD'].includes(body.status)) errors.push({ field: 'status', message: 'Trạng thái khách hàng không hợp lệ.' });
  if (allowAssignedPt && !mongoose.isValidObjectId(body.assignedPtId)) errors.push({ field: 'assignedPtId', message: 'Vui lòng chọn PT phụ trách.' });
  for (const [field, max, label] of [['medicalNotes', 2000, 'Lưu ý sức khỏe'], ['initialGoal', 1000, 'Mục tiêu ban đầu'], ['internalNotes', 2000, 'Ghi chú nội bộ']]) {
...
const createCustomerValidator = (req) => validateCustomerFields(req.body, { allowAssignedPt: req.user.role === 'ADMIN' });
const updateCustomerValidator = (req) => [...idValidator(req), ...validateCustomerFields(req.body, { partial: true })];

function packageValidator(req) {
...
function packageUpdateValidator(req) {
...
const packageIdValidator = (req) => [...idValidator(req), ...(mongoose.isValidObjectId(req.params.packageId) ? [] : [{ field: 'packageId', message: 'Mã gói PT không hợp lệ.' }])];
const customerListValidator = (req) => {
  const errors = listValidator(req);
  if (req.query.status && !['ACTIVE', 'INACTIVE', 'LEAD'].includes(req.query.status)) errors.push({ field: 'status', message: 'Trạng thái khách hàng không hợp lệ.' });
  if (req.query.ptId && !mongoose.isValidObjectId(req.query.ptId)) errors.push({ field: 'ptId', message: 'Mã PT không hợp lệ.' });
  return errors;
};
const packageListValidator = (req) => {
  const errors = [...idValidator(req), ...listValidator(req)];
  if (req.query.status && !['ACTIVE', 'EXPIRED', 'COMPLETED', 'CANCELLED'].includes(req.query.status)) errors.push({ field: 'status', message: 'Trạng thái gói PT không hợp lệ.' });
  return errors;
};
