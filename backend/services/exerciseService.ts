...
type ExerciseResponse = Record<string, unknown> & { id?: string; videoUrl?: string; videos?: IExercise['videos'] };
function normalizeExerciseVideos(value: unknown): ExerciseResponse {
...
async function create(user: AuthenticatedUser, payload: Partial<IExercise>) {
  ...
  return normalizeExerciseVideos(exercise);
}
async function list(user: AuthenticatedUser, query: Record<string, unknown>) {
  ...
  return { items: items.map(normalizeExerciseVideos), meta: { page, limit, total, totalPages: Math.ceil(total / limit) } };
}
async function update(user: AuthenticatedUser, id: string, payload: Partial<IExercise>) {
  ...
  return normalizeExerciseVideos(saved);
}
...
async function get(user: AuthenticatedUser, id: string) { return normalizeExerciseVideos(await getOwned(user, id)); }
