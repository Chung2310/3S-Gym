...
const prescription = Joi.object().when('trackingType', {
  switch: [
    { is: 'STRENGTH', then: Joi.object({ sets: Joi.number().integer().min(1), reps: Joi.string().allow(''), targetWeight: Joi.number().min(0), targetRpe: Joi.number().min(0).max(10), targetRir: Joi.number().min(0), restSeconds: Joi.number().min(0) }).unknown(false) },
    { is: 'BODYWEIGHT', then: Joi.object({ sets: Joi.number().integer().min(1), reps: Joi.string().allow(''), addedWeight: Joi.number().min(0), targetRpe: Joi.number().min(0).max(10), targetRir: Joi.number().min(0), restSeconds: Joi.number().min(0) }).unknown(false) },
    { is: 'CARDIO', then: Joi.object({ durationMinutes: Joi.number().positive(), distanceKm: Joi.number().positive(), targetPaceSecondsPerKm: Joi.number().positive(), targetHeartRate: Joi.number().positive(), inclinePercent: Joi.number().min(0), targetRpe: Joi.number().min(0).max(10) }).unknown(false) },
    { is: 'INTERVAL', then: Joi.object({ rounds: Joi.number().integer().min(1), workSeconds: Joi.number().positive(), restSeconds: Joi.number().min(0), distanceMetersPerRound: Joi.number().positive(), repsPerRound: Joi.number().integer().positive(), targetRpe: Joi.number().min(0).max(10) }).unknown(false) },
    { is: 'MOBILITY', then: Joi.object({ durationMinutes: Joi.number().positive(), reps: Joi.number().integer().positive(), side: Joi.string().valid('LEFT', 'RIGHT', 'BOTH'), targetDiscomfort: Joi.number().min(0).max(10) }).unknown(false) },
  ],
});
