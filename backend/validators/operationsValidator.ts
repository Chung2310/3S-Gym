...
export const createProgressReportSchema: RequestValidationSchema = { body: Joi.object({ customerId: objectId.required(), periodStart: Joi.date().iso().required(), periodEnd: Joi.date().iso().greater(Joi.ref('periodStart')).required(), summary: Joi.string().trim().required(), metrics: Joi.object().pattern(Joi.string(), Joi.number()), sourceVersions: Joi.object().unknown(true) }).messages(commonMessages) };
export const updateProgressReportSchema: RequestValidationSchema = { params: idParams(), body: nonEmptyPatch({ summary: Joi.string().trim(), periodStart: Joi.date().iso(), periodEnd: Joi.date().iso(), metrics: Joi.object().pattern(Joi.string(), Joi.number()), customerId: Joi.forbidden(), ptId: Joi.forbidden(), status: Joi.forbidden(), version: Joi.forbidden(), publishedAt: Joi.forbidden() }) };
