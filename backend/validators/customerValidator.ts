...
import { commonMessages, email, idParams, nonEmptyPatch, objectId, paginationQuery, sixDigitPassword, withDateRange } from './commonValidator.js';
...
export const createCustomerAccountSchema: RequestValidationSchema = { params: idParams(), body: Joi.object({ username: Joi.string().trim().min(3).required(), password: sixDigitPassword.required(), email: email.allow('', null) }).messages(commonMessages) };
